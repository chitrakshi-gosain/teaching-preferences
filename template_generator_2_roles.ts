////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////// THIS SCRIPT IS FOR ////////////////////////////////////////
/////////////////////////// CLASSES WITH 2 KINDS OF CASUAL ACADEMIC ROLES //////////////////////////
//////////////////////////////////////// SUCH AS 1531 / 6080 ///////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
...
function recordPreferences(sheet: ExcelScript.Worksheet, classes: string[], types: string[], counts: number[]) {
  let col = 1;
  const values = [classes, types, counts];
  for (let i = 0; i < classes.length; i++) {
    for (let row = 1; row < 4; row++) {
      const dataRange = sheet.getRange(`${getExcelColumn(col)}${row}:${getExcelColumn(col + 1)}${row}`);
      dataRange.merge();
      dataRange.setValue(values[row - 1][i]);
    }
    col += 2;
  }

  col -= 1;
  let endCol = getExcelColumn(col);
  ...
  for (let i = 0; i < classes.length; i++) {
    tempCol += 1;
    setValueAndColor(
      sheet.getRange(`${getExcelColumn(tempCol)}4:${getExcelColumn(tempCol)}4`), 'Role', COLORS['SKY_BLUE']
    );

    tempCol += 1;
    setValueAndColor(
      sheet.getRange(`${getExcelColumn(tempCol)}4:${getExcelColumn(tempCol)}4`), 'Preference', COLORS['YELLOW']
    );
  }

  sheet.getRange(`A4:${getExcelColumn(tempCol)}4`).getFormat().autofitColumns();
 ...
 */
function createTimetableRows(sheet: ExcelScript.Worksheet, teachingMode: string, currRow: number, classesLen: number, twoTutors: boolean) {
  let dataRange: ExcelScript.Range;
  let col = 3;
  for (let i = 0; i < classesLen; i++) {
    if (twoTutors) {
      for (const row of [currRow, currRow + 1]) {
        dataRange = sheet.getRange(`${getExcelColumn(col)}${row}:${getExcelColumn(col + 1)}${row}`);
        dataRange.merge();
        setValueAndColor(dataRange, '-', COLORS['SALMON']);
      }
    } else {
      dataRange = sheet.getRange(`${getExcelColumn(col)}${currRow}:${getExcelColumn(col + 1)}${currRow + 1}`);
      dataRange.merge();
      setValueAndColor(dataRange, '-', COLORS['SALMON']);
    }
    col += 2;
  ...
  const endCol = getExcelColumn(col - 1);
  dataRange = sheet.getRange(`C${currRow}:${endCol}${currRow + 1}`);
...
function createTimetable(sheet: ExcelScript.Worksheet, schedule: Combinations, classes: string[], types: string[], counts: number[], twoTutors: boolean) {
  ...
  for (let i = 0; i < classes.length; i++) {
    col += 2;
    const dataRange = sheet.getRange(`${getExcelColumn(col)}${currRow}:${getExcelColumn(col + 1)}${currRow}`);
    dataRange.merge();
    dataRange.setValue(classes[i]);
  }
  const endCol = getExcelColumn(col + 1);
  ...
  for (let i = 0; i < maxInPerson; i++) {
    ...
    createTimetableRows(sheet, 'In-person', currRow, classes.length, twoTutors);
    currRow += 2;
  }

  for (let i = 0; i < maxOnline; i++) {
    teachingModeRows['online'].push(currRow);
    createTimetableRows(sheet, 'Online', currRow, classes.length, twoTutors);
    currRow += 2;
  }
...
function addInstructionRows(sheet: ExcelScript.Worksheet, classesLen: number) {
  ...
  addInstructionBlock(sheet, 'What role can you do?', classesLen, classesLen + 6, COLORS['SKY_BLUE'], roles);
  addInstructionBlock(sheet, 'Preference', classesLen + 8, classesLen + 10, COLORS['YELLOW'], preferences);
}
...
function updateWorksheet(sheet: ExcelScript.Worksheet, schedule: Combinations, helpSessions: boolean, twoTutors: boolean) {
  ...
  for (let i = 0; i < classes.length; i++) {
    col += 2;
    const dataRange = sheet.getRange(`${getExcelColumn(col)}1:${getExcelColumn(col + 1)}34`);
    addBorders(dataRange, false, false, ExcelScript.BorderWeight.medium);
  }

  const dataRange = sheet.getRange(`C35:${getExcelColumn(endCol)}35`);
 ...
 */
function main(workbook: ExcelScript.Workbook, course: string, startRow: number, endRow: number, classTimesCol: string, classLocationsCol: string, helpSessions: 'Yes' | 'No', twoTutors: 'Yes' | 'No') {
